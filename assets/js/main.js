  ...
  $(".mobile-submenu-open").on("click", function () {
    ...
    if ($(this).parent().next().css("display") == "none") {
      ...
      $(this)
        ...
        .siblings()
        .children()
        .removeClass("mobile-submenu-show");
    } else {
...
var fnTooltip = function () {
  $(".footnote-ref").each(function () {
    ...
    if (footnote.html()) {
      ...
      $(this)
        .parent()
        .append(
          outer_wrapper.append(inner_wrapper.text(footnote.text().trim()))
        );
    }
  });
  ...
  var position = function () {
    ...
    if ($(window).width() < 640) content.css("width", $(window).width() / 2);
    else content.css("width", 340); // default value
...
var highlight = function () {
  document.querySelectorAll(".chroma>table").forEach((element) => {
